...
def get_oracle_connection():
    if oracledb is None:
        print("Oracle驱动未安装，无法连接")
        return None
    try:
        # Using thin mode or thick mode depending on setup. 
        # Since we installed instant client, thick mode is possible, but thin is easier if supported.
        # oracledb 2.0+ supports thin mode by default which doesn't need instant client for basic stuff.
        # However, for some features instant client is better. Let's try default (thin) first.
        dsn = f"{ORACLE_HOST}:1521/FREEPDB1" # Default service name for gvenzl/oracle-free is FREEPDB1
